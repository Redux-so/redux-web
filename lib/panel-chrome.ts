...
import { SURFACE_BORDER, SURFACE_BORDER_RAISED } from "@/lib/surface-colors";

/** Header row — fixed height with bottom divider. */
export const PANEL_HEADER =
  `flex items-center shrink-0 h-12 border-b ${SURFACE_BORDER}`;
...
export const CHAT_POPOVER_CHROME = `border ${SURFACE_BORDER} bg-[#1d1d1d]`;

/** Secondary button in chat changes bar (Undo all). */
export const CHAT_BTN_SECONDARY =
  `inline-flex items-center justify-center h-8 px-3.5 rounded-md border ${SURFACE_BORDER_RAISED} bg-[#1d1d1d] text-[12px] font-semibold text-white/90 hover:border-[#2e2e2e] transition-colors cursor-pointer select-none disabled:opacity-40 disabled:cursor-not-allowed`;

/** Toolbar chip shell in chat input footer. */
export const CHAT_TOOLBAR_CHIP =
  `rounded-md border ${SURFACE_BORDER_RAISED} bg-[#1d1d1d] shadow-[0_1px_2px_rgba(0,0,0,0.26),0_2px_5px_rgba(0,0,0,0.12)]`;
...
export const CHAT_AVATAR_SHELL =
  "flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full bg-[#1d1d1d]";

/** Send button when input is empty/disabled. */
export const CHAT_SEND_BTN_DISABLED =
  "pointer-events-none shrink-0 flex items-center justify-center w-7 h-7 rounded-md bg-white/[0.08] text-[#666666] opacity-50 cursor-not-allowed";
